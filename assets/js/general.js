var googleUser = {};
// Server URL set up in window.setserver = function(url). Don't change it manually.
var serverURL = '';
// TODO change http to https when will be authrity certificate on server
var serverProtocol = (window.origin === 'http://agario.loc') ? 'http' : 'https';
// Maximum weigth for game to save;
var maxWeight = 0;
...
(function () {
  ...
  function colorToBytes(color) {
    var c = color.slice(1);
    if (c.length === 3) c = c.split("").map(function (a) {
      return a + a
    });
    ...
    switch (packetId) {
      case 0x10: // update nodes
        ...
        for (var i = 0; i < count; i++) {
          ...
          if (cells.mine.includes(killer)) {
            (cells.byId[killed].s < 20 ? console.log('pellet') : food_eaten++);
          }
      ...
      case 0x31: // ffa list
        ...
        for (i = 0; i < count; ++i) {
          ...
          var myPosition = reader.getUint32();
...
///////////////////////////////DEATH SCREEEN
  ...
  function drawLeaderboard() {
    ...
    if (leaderboard.type === "pie") {
	  ...
	  if(parseInt(leaderboard.items[0].myposition) != 0 && parseInt(leaderboard.items[0].myposition) < top_leaderboard_position)
		{		 
  ...
  function cameraUpdate() {
    ...
    if (myCells.length > 0) {
      ...
      stats.score = score;
      // Save max weight for saving to server
  ...
  Cell.prototype = {
    ...
    setSkin: function (value) {   
      if (value) {
        this.skin = value;
        var skinPath = value.replace(/^1\%/, '/free/').replace(/^2\%/, '/level/').replace(/^3\%/, '/premium/') + '.png';
        loadedSkins[this.skin] = new Image();
        loadedSkins[this.skin].src = `${serverProtocol}://${serverURL}/skin/${skinPath}`;
      }
    },
  ...
  function init() {
    ...
    byId("play-btn").addEventListener("click", function () {
      ...
      if (settings.skin) {
        sendPlay("{" + settings.skin + "}" + settings.nick || host);
      } else {
        sendPlay(settings.nick || host);
      }
      maxWeight = 0;
      hideESCOverlay();
	  sec = 0;
  ...
  window.setserver = function (url) {
    ...
    serverURL = (serverURL.indexOf('localhost') !== -1) ? serverURL.replace('localhost/server', 'localhost:3001/server') : serverURL;
  ...
  window.changeSkin = function (a, skinId) {
    settings.skin = a.replace(/^\/skin\//, '').replace(/^free\//, '1%').replace(/^level\//, '2%').replace(/^premium\//, '3%').replace('.png', '');
...
function drawGalleryHandler(skinsP, tab, newSkin = null, searchQuery = searchOut) {
  ...
  for (let i = 0; sortedSkins[i]; i++) {
    ...
    const skinName = `${skin.category}/${skin.name}`;
	 ...
	 if (newSkin == skin._id) 
	 {
			...
			settings.skin = `/skin/${skinName}`.replace(/^\/skin\//, '').replace(/^free\//, '1%').replace(/^level\//, '2%').replace(/^premium\//, '3%').replace('.png', '');
	}
 ...
 {	
      ...
      let disableSkin = '';
      let skinButton = `<div id="${skin._id}" ${_isChanged} class="button-success">use</div>`
  ...
  const galeryBody = `
    <ul id="skins-premium" class="skins-ul">${skins.free}</ul>
  `;
  ...
  if (tab !== 0) document.getElementById("gallery").hide();
}
...
{
	var servers = ["sigmally.com", "u1.sigmally.com"];
	var servers_names = ["US-1", "Europe-2"];
	servers.forEach(getServersData);
	
	function getServersData(item, index) {
		 const server = `${serverProtocol}://${item}`;
			fetch(server + '/server/serversstats', {
			method: 'GET',
			headers: {
			  'Accept': 'application/json',
			  'Content-Type': 'application/json'
		...
		.then(data => {
			console.log(index);
			document.getElementById('option_' + index).innerHTML = servers_names[index] + ' ' + data.body.serverstats.players_current + '/' + data.body.serverstats.players_max;
...
function Share(type)
{
	if(type == 'fb')
	{
		window.open('http://www.facebook.com/sharer.php?u=https://agar-pro.club&quote=Play+to+Sigmally!%20Highest%20Mass%20'+document.getElementById('highest_mass').innerHTML+'%20Top%20Position%20' +document.getElementById('top_leaderboard_position').innerHTML+'%20Time%20Alive%20'+document.getElementById('time_alive').innerHTML + '%20Food%20Eaten%20' + document.getElementById('food_eaten').innerHTML,'pagename','resizable');
	}
	if(type == 'tw')
	{
		window.open('https://twitter.com/intent/tweet?text=Play%20To%20agar-pro.club20Highest%20Mass%20'+document.getElementById('highest_mass').innerHTML+'%20Top%20Position%20' +document.getElementById('top_leaderboard_position').innerHTML+'%20Time%20Alive%20'+document.getElementById('time_alive').innerHTML + '%20Food%20Eaten%20' + document.getElementById('food_eaten').innerHTML,'pagename','resizable');
	}
}